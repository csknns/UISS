...
- (void)testToolbarTintColor; {
    ...
    [self parserTestWithDictionary:dictionary assertionsAfterInvoke:^(NSInvocation *invocation) {
        ...
        UISSAssertEqualSelectors(invocation.selector, @selector(setTintColor:));
...
- (void)testLabelShadowOffset; {
    ...
    [self parserTestWithDictionary:dictionary assertionsAfterInvoke:^(NSInvocation *invocation) {
        ...
        UISSAssertEqualSelectors(invocation.selector, @selector(setShadowOffset:));

        CGSize shadowOffset;
        [invocation getArgument:&shadowOffset atIndex:2];
        XCTAssertEqualCGSize(shadowOffset, CGSizeMake(1, 1));

        XCTAssertEqualCGSize([[UILabel appearance] shadowOffset], CGSizeMake(1, 1));
    }];
}
